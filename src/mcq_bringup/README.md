...
The simulator publishes the kart's sensors (`/gnss/fix` at 20 Hz and 80 ms late, `/imu/data_raw` at 200 Hz, wheel speeds and steering angle as measured) and the truth on `/ego_truth`. `/ego_state` is that same truth republished, standing in for `mcq_localization`; `ego_from_sim:=false` stops it once the estimator runs.
