...
2026-09-22: the simulator publishes sensors rather than answers (`mcq_sim/sensors.py`): GNSS at 20 Hz delayed 80 ms with noise by fix status and a scheduled status that can degrade to float and to no fix, an IMU at 200 Hz with a bias that walks, and wheel speeds and a steering angle quantized by their encoders with the rear wheels reading high under throttle. The true state moves to `/ego_truth`, which nothing on the kart will ever publish and which the state estimator is scored against. Until `mcq_localization` exists the simulator republishes the truth as `/ego_state` under `publish_ego_state`, so the graph keeps driving. Two decisions: a sample carries the time it was measured rather than the time it was published, because the estimator has to apply a fix that is already 80 ms old; and our three fix statuses reach `sensor_msgs/NavSatFix`, which has no RTK float or fixed, as no fix, fix and GBAS fix, with the distinction the estimator gates on travelling in the covariance. That mapping is a guess about `ublox_dgnss` and must be checked against the real receiver in Phase 1. Map frame conversions both ways live in `mcq_sim/geodesy.py`, shared with the survey tool; the tangent-plane shortcut for the inverse was dropped because it warps 3 cm at 500 m from the datum, which is a third of the Phase 1 pose budget.
...
`mcq_track/track_server` owns the loaded planning map. Its dependency-free C++ `mcq_track_core` builds a cumulative chord-length table, periodic cubic splines for a closed track (natural splines for an open track), and a balanced KD-tree over centerline samples. Cartesian-to-Frenet lookup projects onto the two segments adjoining the nearest sample, matching the Python reference. KD-tree lookup is typically logarithmic, with linear worst case; the spline interval lookup is logarithmic. Neither is strictly constant time.

The geofence uses the polygon between the surveyed edge polylines, including the hole inside a closed circuit and the end caps of an open track. It computes signed distance to the polygon and inflates by `geofence_margin` (initially 0.5 m). `GeofenceState.distance_to_edge` is the signed clearance to this inflated polygon. The largest eigenvalue of the planar x/y position covariance must not exceed `position_variance_max` (initially 0.25 m^2). Non-finite, asymmetric or non-positive-semidefinite planar covariance, invalid map coordinates, and stale poses also fail the gate. The controller consumes the verdict directly and requests urgent stop in AUTO if it is missing, stale or violating. In RC, missing or unsafe inputs disable axis authority without requesting an urgent stop; the real gateway honors Jetson stop requests even in RC. The planner subscribes to the same verdict for its braking trajectory.

`mcq/load_track` loads a directory containing `track.csv` and `track.yaml`. Raceline integration is deferred to issue #5: configured YAML `raceline` entries and nonempty received raceline arrays are rejected until spline and candidate-path containment against surveyed boundaries is validated. Parsing and geometry validation finish before the live model is replaced; failure returns the active track id and keeps the last good map. A successful load immediately reevaluates the current pose and publishes the new `TrackModel`. The model also publishes at 10 Hz with reliable transient-local depth-one QoS. `planner_node` constructs its geometry from this message and never reads a track file. The simulator keeps a separate physical-world track; reloading a survey must not move the simulated pavement.

Frenet ownership for issues #2 and #3: the estimator remains the sole publisher of `EgoState`, including `s` and `d`. It will consume `TrackModel`, instantiate the exported `mcq_track_core` geometry and fill those fields before publishing. The track server does not republish `EgoState` or create a feedback topic. Until the estimator lands, the simulator's truth-derived `s` and `d` retain their physical-world meaning; the planner and geofence independently project x/y onto the active surveyed map so a shifted survey is detected.
