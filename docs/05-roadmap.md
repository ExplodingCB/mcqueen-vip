...
Progress 2026-09-12: done are the repository structure, the x86 Dockerfile, CI, `mcq_msgs`, the DBC with generated codecs, the gateway core with the software half of the fault-injection list passing on the host, the C control core, the simulator closing the loop on the synthetic oval in both `FOLLOW` and `BOUNDARY` modes with zero interventions, the ROS 2 graph (simulator, planner and controller nodes) driving the same oval in CI with the session recorded to MCAP, the SocketCAN gateway bridge, and the survey tool for Phase 1. Open are the MCAP replay tests (they need the first real logs), the gateway board support and the bench rig.
