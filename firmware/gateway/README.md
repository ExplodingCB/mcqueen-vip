...
Portable core of the gateway described in [docs/04-safety.md](../../docs/04-safety.md): the mode state machine, the heartbeat check, the per-frame limits, the CRSF receiver parser, the CRC and the `JETSON_COMMAND` frame codec. Plain C11, no allocation, no hardware headers, so every fault-injection item that is software runs on the host.
...
| `src/crsf.c` | Streaming RC channel decoding, configured failsafe, timeout and gateway input adapter |
| `test/test_crsf_gateway.c` | Raw receiver frames through fault-injection items 8, 11 and 12 |
...
Receiver setup, board integration and the bench fault-injection mapping are in [CRSF.md](CRSF.md).

Board support (CAN peripheral, receiver UART, encoder and actuator drivers, the main loop calling `gw_step` at a fixed rate) is added next to this core once the board is chosen (STM32H7 or Teensy 4.1 carrier, or a comma panda). Keep this directory free of hardware headers.
